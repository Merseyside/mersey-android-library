...
class NestedModelIModelListManager<Parent, Model, InnerData, InnerAdapter>(
    modelList: ModelList<Parent, Model>,
    override val adapterActions: NestedAdapterActions<Parent, Model, InnerData, InnerAdapter>,
) : IModelListManager<Parent, Model>(modelList, adapterActions),
    ...
    override suspend fun updateModel(model: Model, item: Parent): Boolean {
        return super<INestedIModelListManager>.updateModel(model, item)
    }
}